...
document.getElementById('location').addEventListener('click', () => {
    ...
    // 내 위치 찾기 버튼 비활성화 후, 범죄구간 버튼 활성화
    toggleLocationButton(false);
    toggleCrimeLayerButtons(true);
});

// 범죄구간 버튼 활성화/비활성화
function toggleCrimeLayerButtons(enable) {
    const crimeButtons = ['toggleKidLayer', 'toggleElderLayer', 'toggleRapeLayer'];
    crimeButtons.forEach(buttonId => {
        const button = document.getElementById(buttonId);
...
// 내 위치 찾기 버튼 활성화/비활성화
function toggleLocationButton(enable) {
    const locationButton = document.getElementById('location');
    locationButton.disabled = !enable;
}
